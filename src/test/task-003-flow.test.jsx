...
describe('TASK-003: feed, reactions, comments and navigation', () => {
  ...
  it('opens feed after auth + onboarding flow', async () => {
    ...
    expect(screen.getAllByRole('button', { name: /Feed/i })[0]).toHaveAttribute('aria-current', 'page');
  });
  ...
  it('adds a comment and shows it in UI', async () => {
    ...
    await user.type(await screen.findByPlaceholderText(/add a comment/i), 'Новый комментарий из теста');
  ...
  it('blocks too long comment and shows validation message', async () => {
    ...
    expect(await screen.findByText(new RegExp(`maximum length is ${COMMENT_MAX_LENGTH}`, 'i'))).toBeInTheDocument();
  }, 10000);
