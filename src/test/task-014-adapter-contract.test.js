...
describe('TASK-014: feed adapter response contract', () => {
  it('returns feed items with required clip fields and stable primitive shapes', () => {
    ...
    expect(firstClip).toEqual(expect.objectContaining(Object.fromEntries(
      REQUIRED_CLIP_FIELDS.map((field) => [field, expect.anything()])
    )));
  ...
  it('returns profile with required fields and numeric counters', () => {
    ...
    expect(profile).toEqual(expect.objectContaining(Object.fromEntries(
      REQUIRED_PROFILE_FIELDS.map((field) => [field, expect.anything()])
    )));
