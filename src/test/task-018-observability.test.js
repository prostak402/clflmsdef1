...
describe('TASK-018: request metrics and key events', () => {
  ...
  it('logs feed load event and successful request timing metric', () => {
    ...
    const eventLog = logger.mock.calls.find(([prefix, event]) => (
      prefix === '[api-event]' && event?.name === 'feed.load.requested'
    ));
    expect(eventLog).toBeTruthy();

    const metricLog = logger.mock.calls.find(([prefix, event]) => (
      prefix === '[api-metric]' && event?.endpoint === 'GET /feed'
    ));
  ...
  it('logs comment submit event payload', () => {
    ...
    const eventLog = logger.mock.calls.find(([prefix, event]) => (
      prefix === '[api-event]' && event?.name === 'comment.submit.requested'
    ));
