...
- legacy-поле `time` допускается только как fallback-источник для формирования `timeLabel` в клиентском normalizer.
- legacy-поле `user` допускается только как fallback-источник для `authorName`.

### 2.4.2 Поля модерации комментариев (future-ready)
...
- `approved` комментарии видны публично.
- `pending/rejected` скрываются из публичной выдачи, но доступны автору и `admin`.
- изменение `moderationStatus` доступно только `admin`.
